...
export default function CalendarPage() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        gridCells.push(<div key={`day-${d}`} onClick={() => handleDayClick(d)} className={`bg-card border border-border min-h-28 p-2.5 flex flex-col justify-between hover:bg-secondary/40 transition-all cursor-pointer group ${hasTransactions ? "hover:border-primary/50" : ""}`}>
    ...
    return (<div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Grid */}
      {loading ? (<div className="py-20 text-center flex flex-col items-center justify-center gap-3 bg-card border border-border rounded-2xl shadow-sm">
          <Loader2 className="h-8 w-8 text-primary animate-spin"/>
          <span className="text-sm text-muted-foreground">Compiling calendar view...</span>
        </div>) : (<div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
          {/* Days of week header */}
      ...
      {drawerOpen && selectedDate && (<div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-end p-0 md:p-4 backdrop-blur-sm">
          <div className="bg-card border-l border-border h-full md:h-[calc(100vh-2rem)] w-full max-w-md md:rounded-3xl overflow-hidden shadow-2xl flex flex-col relative animate-in slide-in-from-right duration-300">
            {/* Header */}
