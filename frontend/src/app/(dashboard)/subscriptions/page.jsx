...
export default function SubscriptionsPage() {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const err = error;
            setFormError(err.response?.data?.detail || "Failed to save subscription.");
        }
        finally {
            setSubmitting(false);
        }
        ...
        {loading ? (<div className="py-20 text-center flex flex-col items-center justify-center gap-3">
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-border text-sm">
                ...
                return (<tr key={sub.id} className="hover:bg-secondary/20 transition-colors">
                      <td className="px-6 py-4 font-semibold">{sub.name}</td>
                      <td className="px-6 py-4">
                        <span className="inline-block py-1 px-2.5 bg-secondary text-foreground text-xs font-semibold rounded-full">
                          {sub.frequency}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        {soon ? (<span className="inline-flex items-center gap-1 py-1 px-2 bg-amber-500/10 text-amber-500 text-[10px] font-bold rounded-lg border border-amber-500/20 uppercase">
                            Due soon
                          </span>) : (<span className="text-xs text-slate-500">-</span>)}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-1.5">
                          <button onClick={() => handleOpenEditModal(sub)} className="p-1.5 rounded-lg text-slate-500 hover:text-foreground hover:bg-secondary cursor-pointer" title="Edit">
                            <Edit2 className="h-4 w-4"/>
                          </button>
                          <button onClick={() => handleDelete(sub.id)} className="p-1.5 rounded-lg text-slate-500 hover:text-destructive hover:bg-destructive/10 cursor-pointer" title="Delete">
